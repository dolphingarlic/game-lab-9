...
class Test_2_KeeperPlacement(unittest.TestCase):
    ...
    def test_02(self): 
        ...
        # Place keeper on rock, then on path
        # using resources/maps/zoo2-horizontal-path.json
        verify_case('2-2a')
    ...
    def test_03(self): 
        # Make edge of keeper contiguous with edge of path, then two more clicks
        # using resources/maps/zoo7-ugly-path.json
        verify_case('2-3a')
    ...
    def test_04(self): 
        # event sequence: keeper, None, loc, None, aim
        # using resources/maps/zoo2-horizontal-path.json
        verify_case('2-4a')
    ...
    def test_05(self): 
        ...
        # Place keeper on path; starts with null mouse
        # using resources/maps/zoo2-horizontal-path.json
        verify_case('2-5b')
...
def unique_order(formations):
    return sorted([(*tuple(form["loc"]), *tuple(form["size"]), form["texture"], *tuple(form.get("aim_dir", [None]) or [None])) for form in formations])
